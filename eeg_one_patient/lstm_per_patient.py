...
def data_generator_one_patient(main_folder, patient_number,size_in, leaveout_sample, isTrain=True):
    ...
    if (isTrain):
        ...
        for sample in list_samples:
            ...
            if (sample != ('chb' + str(patient_number).zfill(2) + '_' + str(leaveout_sample).zfill(2) +'_seg.mat')):
                print(sample)
	...
	Y[Y==2]=0
        ...
        mat_var = loadmat(patient_folder + '/' + 'chb' + str(patient_number).zfill(2) + '_' + str(leaveout_sample).zfill(2) +'_seg.mat')
        X =  mat_var['total_images']
        Y = mat_var['total_labels']
        X = np.reshape(X, (X.shape[0], size_in, 23, 1))
	Y[Y==2]=0
        #Y = np_utils.to_categorical(Y, 2)
        return X, Y
        # yield X_test, Y_test
...
#    main_folder = '/home/gustavo/'
    ...
    model = Sequential()
    model.add(TimeDistributed(Conv1D(nb_filter=30, filter_length=10), input_shape=(size_in,num_channels,1)))
    model.add(Activation('relu'))
    model.add(TimeDistributed(MaxPooling1D()))
    model.add(Dropout(0.2))
    model.add(TimeDistributed(Conv1D(nb_filter=15, filter_length=6)))
    model.add(Activation('relu'))
    #model.add(TimeDistributed(MaxPooling1D()))
    model.add(Dropout(0.2))
    ...
    model.compile(loss='categorical_crossentropy',
                  optimizer=sgd)
    los =4
    X_train, Y_train = data_generator_one_patient(main_folder=main_folder, patient_number=pat_num, size_in=size_in, leaveout_sample=los,
                                                  isTrain=True)

    print(X_train.shape)
    print(Y_train.shape)

    X_test, Y_test = data_generator_one_patient(main_folder=main_folder, patient_number=pat_num, size_in=size_in, leaveout_sample=los,
                                                  isTrain=False)
    ...
    print('Test # false positives:', float(metrics_t[1])/(float(X_train.shape[0]/30.0)))
    ...
    print('fpr:', float(metrics_test[1])/(float(X_test.shape[0]/30.0)))
    ...
    with open('patient.pickle', 'w') as f:  # Python 3: open(..., 'wb')
