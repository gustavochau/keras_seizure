...
def data_generator_one_patient(main_folder, patient_number,size_in,balance=False,bal_ratio=4):
    nb_classes = 2
...
#    main_folder = '/home/gchau/Documents/data/epilepsia_data/Data_segmentada_ds1/'
    ...
    for lop in list_all_patients:
        ...
        for zz in range(0,num_realizations):
            nombre_pesos = 'c1d_only_sparse_pat' + str(lop) + '_weights.h5'
            ...
            print('Train false positive rate:', float(metrics_t[1]) / (float(X_train.shape[0])  / 3600.0))

            resumen_train[zz, 0] = metrics_t[0]
            resumen_train[zz, 1] = float(metrics_t[1]) / (float(X_train.shape[0])  / 3600.0)
            ...
            print('Test false positive rate:', float(metrics_test[1]) / (float(X_test.shape[0])  / 3600.0))

            resumen_test[zz, 0] = metrics_test[0]
            resumen_test[zz, 1] = float(metrics_test[1]) / (float(X_test.shape[0])  / 3600.0)
...
savemat(file_name='cnn_1d_only_todo.mat', mdict=variables_save)
