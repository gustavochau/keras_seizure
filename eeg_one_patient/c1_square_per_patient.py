    ...
    if (isTrain):
		...
		#print(X_train.shape)
                ...
                if balance:
                    ...
                    sel_ind_negative = random.sample(ind_negative, num_positive[0] * bal_ratio)
                    not_selected = list(set(ind_negative) - set(sel_ind_negative))  # which rows to remove
                    X_train = np.delete(X_train, not_selected, 0)
                    Y_train = np.delete(Y_train, not_selected, 0)
...
if __name__ == "__main__":
    ...
    for lop in list_all_patients:
        ...
        for idx, los in enumerate(list_seizures):
            ...
            name_pretrained_weights = 'c1_energy_lstm_pretrained_pat' + str(lop) + '_weights.h5'
            ...
            name_save_weights = model.load_weights(name_save_weights)
            ...
            results_summary[lop,0,idx] = sensitivity
            results_summary[lop, 1, idx] = fpr
            ...
            results_summary[lop,2,idx] = sensitivity
            results_summary[lop,3, idx] = fpr
