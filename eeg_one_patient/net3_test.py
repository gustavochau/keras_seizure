'''Trains a 1D-CNN on the data of one patient, predicts over that patient
'''
...
# compute sensitivity and number of false positive
...
# take all positive segments and the same number of random negative segments
...
def folder_to_dataset(folder):
    ...
    X = np.zeros(shape=(num_samples, 512, 23)) #dataset in theano format
    ...
    for index, file in enumerate(list_files):
        ...
        X[index,:] = (mat_var['x']).reshape(512,  23)
...
if __name__ == "__main__":
    ...
    model = Sequential()
    model.add(Conv1D(nb_filter=120, filter_length=4, input_shape=(512, 23)))
    model.add(Activation('relu'))
    model.add(MaxPooling1D())
    model.add(Conv1D(nb_filter=120, filter_length=3, input_shape=(512, 23)))
    model.add(Activation('relu'))
    model.add(MaxPooling1D())
    model.add(Flatten())
    ...
    # Fit the model
    model.fit(X_train, Y_train, validation_split=0.1, shuffle=True, epochs=epochs, batch_size=100)
