...
if __name__ == "__main__":
 ...
 #       name_pretrained_weights = 'cross2d_norm_polar_pat' + str(patient_number) + '_weights_dropall.h5'
        ...
        for idx,los in enumerate(list_seizures):
            ...
            print('Train # false positives:', float(fp)/(float(X_train.shape[0])*1.0/3600.0))

            resumen_train[idx, 0] = sensitivity
            resumen_train[idx, 1] = float(fp)/(float(X_train.shape[0])*1.0/3600.0)
            ...
            print('Test # false positives:', float(fp)/(float(X_test.shape[0])*1.0/3600.0))

            resumen_test[idx, 0] = sensitivity
            resumen_test[idx, 1] = float(fp)/(float(X_test.shape[0])*1.0/3600.0)
        
        
        promedio_train = np.average(resumen_train, axis=0)
        promedio_test = np.average(resumen_test, axis=0)

        results_summary[patient_number,0] = promedio_train[0]
        results_summary[patient_number,1] = promedio_train[1]
        results_summary[patient_number,2] = promedio_test[0]
        results_summary[patient_number,3] = promedio_test[1]
    ...
    savemat(file_name = 'cnn_2d_per_patient.mat', mdict=variables_save)
